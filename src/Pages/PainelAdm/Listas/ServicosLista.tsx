...
const ServicosLista = () => {
  ...
  const atualizaDados = (id: number) => {
    ...
    const servicoAlterado = retorno.find(
      (servico: ServicosDataProps) => servico.id === id
    );
    if (servicoAlterado) {
  ...
  async function fetchAllServicos(): Promise<ServicosNormalizados[]> {
    ...
    if (token) {
      ...
      while (currentPage <= pages) {
        const { url, options } = GET_INNER_ALL(
          "servico",
          "usuario",
          currentPage
        );
    ...
    data && (
      <div>
        <div className={styleLista.cabecalho}>
          ...
          <ExportToExcel<ServicosNormalizados>
            data={data}
            fileName="Servicos"
            fetchAllData={fetchAllServicos}
          />
        </div>
        <Lista<ServicosNormalizados>
          tbody={listaFiltrada ? listaFiltrada : servicos}
