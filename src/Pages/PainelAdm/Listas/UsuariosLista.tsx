...
const UsuariosLista = () => {
  ...
  function normalizaUsuarios(usuarios: UsuariosDataProps[]) : UsuariosNormalizados[]{
  ...
  useEffect(() => {
    async function getUsuarios() {
      ...
      if(token){
        ...
        if (response && !response.ok) console.log("Ocorreu um erro ao buscar usuarios");
        else {
  ...
  const atualizaDados = (id: number) => {
    const usuarioAlterado = data.find((user: UsuariosDataProps) => user.id === id);
    if (usuarioAlterado) {
  ...
  <div>
    <div className={styleLista.cabecalho}>
      ...
      <ExportToExcel<UsuariosNormalizados> data={usuarios} fileName="Usuarios" />
      </div>
    </div>
    <Lista<UsuariosNormalizados> tbody={listaFiltrada ? listaFiltrada : usuarios} confirmDelete={confirmDelete} atualizaDados={atualizaDados}/>
    {modal === "confirmDelete" && (
          <Confirm
